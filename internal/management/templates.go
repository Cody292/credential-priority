...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        @media (max-width:720px) { body{padding:12px}.container{padding:20px}.grid,.config-detail-grid,.provider-selection-grid,.priority-rule-grid{grid-template-columns:1fr}.topbar{align-items:center}.topbar-actions{flex-wrap:wrap}.toast-root{width:100%; max-width:100%; order:2}.tabs{display:grid;grid-template-columns:1fr 1fr}.switch-line{display:block}.config-control-row{display:block}.config-control-row select{width:100%}.config-selection-row{flex-direction:column}.modal-selection-row{flex-direction:column} #configPanel{max-width:100%} #providerModeSelect{width:100%} }
    </style>
</head>
<body>
    <div class="container">
                ...
                <form id="configForm" class="card config-main-card">
                    ...
                    <div id="configPriorityRulesCard" class="card priority-rules-card rules-disabled" data-priority-rules="provider-separated">
                        <div class="priority-rules-header">
                            ...
                            <label class="switch"><input id="priorityRulesEnabled" name="priority_rules.enabled" type="checkbox" onchange="syncPriorityRulesEnabled()"><span class="slider"></span></label>
                        </div>
    ...
    <script>
        ...
        function fillPriorityRuleForm(rules){const merged={enabled:rules&&typeof rules.enabled==="boolean"?rules.enabled:defaultPriorityRuleConfig.enabled,antigravity:{...defaultPriorityRuleConfig.antigravity,...((rules&&rules.antigravity)||{})},codex:{...defaultPriorityRuleConfig.codex,...((rules&&rules.codex)||{})}};writeRuleValue("enabled",merged.enabled);writeRuleValue("antigravity.start_priority",merged.antigravity.start_priority);writeRuleValue("codex.start_priority",merged.codex.start_priority);writeRuleValue("codex.free_depleted_priority",merged.codex.free_depleted_priority);writeRuleValue("codex.free_depleted_disabled",merged.codex.free_depleted_disabled);writeRuleValue("codex.paid_depleted_keeps_enabled",merged.codex.paid_depleted_keeps_enabled);syncPriorityRulesEnabled();}
        function syncPriorityRulesEnabled(){const enabled=readRuleChecked("enabled");const card=document.getElementById("configPriorityRulesCard");if(card){card.classList.toggle("rules-disabled",!enabled);}document.querySelectorAll(".priority-rule-field input").forEach(function(input){input.disabled=!enabled;});}
        function restoreDefaultPriorityRules(){if(priorityRulesRestoreCoolingDown){return;}if(!window.confirm(textFor("restoreDefaultRulesConfirm"))){return;}fillPriorityRuleForm(defaultPriorityRuleConfig);showToast(textFor("priorityRulesRestored"),"success");startPriorityRulesRestoreCooldown();}
