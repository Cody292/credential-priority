...
<body>
    <div class="container">
        <div class="topbar">
            <h1><span data-i18n="pageTitle">凭证优先级管理</span><span class="version-badge">v1.0.10</span></h1>
            ...
            <section id="configPanel" hidden>
                ...
                <form id="configForm" class="card config-main-card">
                    ...
                    <div id="providerSection" class="section" style="margin-top:0; display:flex; flex-direction:column; gap:12px;">
                        <div class="hint future-providers-hint" data-i18n="futureProvidersHint" style="margin-top:4px; margin-bottom:8px; font-size:12px; color:var(--muted);">目前支持 Antigravity、Codex 与 xAI 凭证，排序规则已按提供商独立分开。后续可能支持：Anthropic、Kimi、Vertex JSON。</div>
                    ...
                    <div id="configPriorityRulesCard" class="card priority-rules-card rules-disabled" data-priority-rules="provider-separated">
                        ...
                        <details class="priority-rules-provider" data-rule-provider="xai">
                            <summary class="priority-rules-summary"><strong>xAI</strong><span class="priority-rules-expand-hint"><span data-i18n="clickToExpand">点击展开</span><svg class="priority-rules-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"></path></svg></span></summary>
                            <ul><li data-i18n="xaiRuleFresh">只使用本轮探测到的可信额度信号；不按套餐名排序。</li><li data-i18n="xaiRuleFree">免费额度耗尽时写入配置优先级并默认禁用；仍有额度时进入正优先级唯一排序。</li><li data-i18n="xaiRuleWeekly">仅周限额耗尽时写入配置优先级且不禁用；周与月均耗尽时写入配置优先级并默认禁用。</li></ul>
                            <div class="priority-rule-grid">
                                <label class="priority-rule-field"><span data-i18n="priorityRuleStartPriority">可用凭证起始优先级</span><input name="priority_rules.xai.start_priority" type="number" min="1" value="100"></label>
                                <label class="priority-rule-field"><span data-i18n="priorityRuleFreeDepletedPriority">免费额度为 0 时优先级</span><input name="priority_rules.xai.free_depleted_priority" type="number" value="-1"></label>
                                <label class="priority-rule-toggle priority-rule-field"><span data-i18n="priorityRuleFreeDepletedDisabled">免费额度为 0 时禁用</span><span class="switch priority-rule-switch"><input name="priority_rules.xai.free_depleted_disabled" type="checkbox" checked><span class="slider"></span></span></label>
                                <label class="priority-rule-field"><span data-i18n="priorityRuleWeeklyDepletedPriority">仅周限额耗尽时优先级</span><input name="priority_rules.xai.weekly_depleted_priority" type="number" value="-1"></label>
                                <label class="priority-rule-field"><span data-i18n="priorityRuleMonthlyWeeklyDepletedPriority">周与月均耗尽时优先级</span><input name="priority_rules.xai.monthly_and_weekly_depleted_priority" type="number" value="-1"></label>
                                <label class="priority-rule-toggle priority-rule-field"><span data-i18n="priorityRuleMonthlyWeeklyDepletedDisabled">周与月均耗尽时禁用</span><span class="switch priority-rule-switch"><input name="priority_rules.xai.monthly_and_weekly_depleted_disabled" type="checkbox" checked><span class="slider"></span></span></label>
                            </div>
                        </details>
                    </div>
                </div>
    ...
    <div id="providerModal" class="modal-backdrop" hidden>
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="providerModalTitle">
            ...
            <p class="hint future-providers-hint" data-i18n="futureProvidersHint" style="margin-bottom:16px; font-size:12px; color:var(--muted);">目前支持 Antigravity、Codex 与 xAI 凭证，排序规则已按提供商独立分开。后续可能支持：Anthropic、Kimi、Vertex JSON。</p>
    ...
    <script>
        ...
        const defaultPriorityRuleConfig={enabled:false,antigravity:{start_priority:100},codex:{start_priority:100,free_depleted_priority:-1,free_depleted_disabled:true,paid_depleted_keeps_enabled:true},xai:{start_priority:100,free_depleted_priority:-1,free_depleted_disabled:true,weekly_depleted_priority:-1,monthly_and_weekly_depleted_priority:-1,monthly_and_weekly_depleted_disabled:true}};
        ...
        function supportedProviderOptions(options){const labels=new Map(options.map(function(item){return [item.value,item.label];}));return ["antigravity","codex","xai"].map(function(value){return {value:value,label:labels.get(value)||getProviderDisplayName(value)};});}
        ...
        function renderCheckboxes(rootId, options, kind, selected){const root=document.getElementById(rootId);root.textContent="";for(const provider of options){const val=provider.value.toLowerCase();if(val==="antigravity"||val==="codex"||val==="xai"){root.appendChild(createProviderCheckbox(provider,kind,selected||[]));}}}
        ...
        function readPriorityRuleConfig(){return {enabled:readRuleChecked("enabled"),antigravity:{start_priority:readRuleNumber("antigravity.start_priority")},codex:{start_priority:readRuleNumber("codex.start_priority"),free_depleted_priority:readRuleNumber("codex.free_depleted_priority"),free_depleted_disabled:readRuleChecked("codex.free_depleted_disabled"),paid_depleted_keeps_enabled:readRuleChecked("codex.paid_depleted_keeps_enabled")},xai:{start_priority:readRuleNumber("xai.start_priority"),free_depleted_priority:readRuleNumber("xai.free_depleted_priority"),free_depleted_disabled:readRuleChecked("xai.free_depleted_disabled"),weekly_depleted_priority:readRuleNumber("xai.weekly_depleted_priority"),monthly_and_weekly_depleted_priority:readRuleNumber("xai.monthly_and_weekly_depleted_priority"),monthly_and_weekly_depleted_disabled:readRuleChecked("xai.monthly_and_weekly_depleted_disabled")}};}
        function fillPriorityRuleForm(rules){const merged={enabled:rules&&typeof rules.enabled==="boolean"?rules.enabled:defaultPriorityRuleConfig.enabled,antigravity:{...defaultPriorityRuleConfig.antigravity,...((rules&&rules.antigravity)||{})},codex:{...defaultPriorityRuleConfig.codex,...((rules&&rules.codex)||{})},xai:{...defaultPriorityRuleConfig.xai,...((rules&&rules.xai)||{})}};writeRuleValue("enabled",merged.enabled);writeRuleValue("antigravity.start_priority",merged.antigravity.start_priority);writeRuleValue("codex.start_priority",merged.codex.start_priority);writeRuleValue("codex.free_depleted_priority",merged.codex.free_depleted_priority);writeRuleValue("codex.free_depleted_disabled",merged.codex.free_depleted_disabled);writeRuleValue("codex.paid_depleted_keeps_enabled",merged.codex.paid_depleted_keeps_enabled);writeRuleValue("xai.start_priority",merged.xai.start_priority);writeRuleValue("xai.free_depleted_priority",merged.xai.free_depleted_priority);writeRuleValue("xai.free_depleted_disabled",merged.xai.free_depleted_disabled);writeRuleValue("xai.weekly_depleted_priority",merged.xai.weekly_depleted_priority);writeRuleValue("xai.monthly_and_weekly_depleted_priority",merged.xai.monthly_and_weekly_depleted_priority);writeRuleValue("xai.monthly_and_weekly_depleted_disabled",merged.xai.monthly_and_weekly_depleted_disabled);syncPriorityRulesEnabled(false);}
